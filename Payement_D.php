    ...
    try {
        ...
        if (isset($_POST['inscription'])) {
            ...
            if (empty($mail)) {
                ...
                $req_mail = $DB->prepare("SELECT mail FROM donneurs WHERE mail = ?");
            ...
            if ($valid) {
                ...
                $req_insert = $DB->prepare("INSERT INTO donneurs (nom, prenom, mail, prix) VALUES (?,?,?,?)");
                $req_insert->execute(array($nom, $prenom, $mail, $prix));
                $_SESSION['prix'] = $prix;

                header('Location: Payement_D_2.php');
                exit;
            }
        }
    } catch (PDOException $e) {
